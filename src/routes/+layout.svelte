...
{#if !loadGodot && csr}
  <span
    class='text-white absolute top-2 right-5 z-10 hover:underline cursor-pointer hidden lg:inline'
    on:click={enableGodot}
    on:keydown={enableGodot}
  >
    Load <Emoji emoji='🤖' />Godot background scene in browser
  </span>
{/if}
<div 
  class='flex flex-col min-h-screen'
  data-semi-transparent={loadGodot?'true':'false'}
>
  <div
    class='wrapper flex-1 flex flex-col relative'
  >
    ...
    {:else}
      <main
        class='max-w-full ml-auto mr-auto flex flex-col flex-1 w-[1000px]'
      >
        <section
