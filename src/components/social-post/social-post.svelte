...
{#if post !== null}
  ...
  >
    <div
      class='post flex flex-col lg:max-w-[320px] bg-white dark:bg-zinc-900 lg:rounded-t-xl overflow-hidden flex flex-col lg:w-auto w-[100%]'
    >
      <span
        class='text-sm pl-4 pr-4 pt-4'
      >
        <a 
      ...
      <em
        class='italic p-4 pt-2 pb-0 mb-[-10px] text-zinc-500 dark:text-zinc-400'
      >
        {post.date}
      </em>
      {#if showTitle}
        <span 
          class='font-bold p-4 pb-0 mb-[-10px]'
        >
          {@html convertEmojiToImages(post.title)}
        </span>
      {/if}
      <span
        class={`p-4 ${truncateContent?'overflow-hidden lg:max-h-[200px]':''}`}
      >
      ...
      {#if truncateContent}
        <div
          class='z-10 linear-gradient-black-to-alpha mt-[-100px] leading-[160px] h-[100px] text-center relative hidden lg:block'
        >
          <a
            class='hover:underline hidden lg:inline show-more text-white'
      ...
      >
        <a
          class='bg-zinc-100 dark:bg-zinc-800 p-4 pt-3 pb-2 text-md block hover:underline text-blue-600 dark:text-red-300 text-xs'
          href={post.originalUrl}
        >
...
<style scoped>
  /* okay okay, I caved and used CSS again, well, at least this is a utility class, right??? */
